## Hi there 👋
# Hi, I'm [Reagan Galo] 👋

CS Student | Java Learner | Backend & Security Enthusiast

## About Me

I'm currently transitioning from C++ to Java through a structured 16-week learning journey. Former C++ programmer, now diving deep into the Java ecosystem and backend development fundamentals.

## 🎯 Current Focus

- **Week [3]/16**: [Current topic - e.g., Object-Oriented Programming]
- Building core Java skills via University of Helsinki MOOC
- Exploring cybersecurity basics (confidentiality, integrity, availability)
- Preparing for Spring Boot and framework learning

## 🛠️ Tech Stack

**Languages**
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=c%2B%2B&logoColor=white)

**Tools & Environment**
![IntelliJ IDEA](https://img.shields.io/badge/IntelliJ_IDEA-000000.svg?style=flat&logo=intellij-idea&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-100000?style=flat&logo=github&logoColor=white)

## 📚 Learning Roadmap

| Phase | Status | Topics |
|-------|--------|--------|
| 1: Foundations | 🟡 In Progress | Syntax, OOP, Collections |
| 2: Core Java | ⬜ Planned | Exceptions, I/O, Generics |
| 3: Advanced | ⬜ Planned | Concurrency, JVM, Performance |
| 4: Frameworks | ⬜ Planned | Spring Boot, Testing |

## 🔨 Projects

*Coming soon as I build through my learning journey...*

## 📈 GitHub Stats

![Your GitHub stats](https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=default)

## 🤝 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](YOUR_LINKEDIN_URL)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:YOUR_EMAIL)

---

*"Building projects one commit at a time."*
